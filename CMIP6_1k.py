...
def select_data():
    ...
    # **修正 GWL 變數**
    GWL_options = {1: "1.5°C", 2: "2°C", 3: "3°C", 4: "4°C"}
...
def process_data():
    ...
    for i, modelname in enumerate(model):
        bucket_path = bucket_path0 + scenarios[i] + '_' + modelname + '.zarr'
        file0 = bucket_path + '/'
        ...
        if pd.isna(center_yr[i]):
            log_message(f"⚠️ 跳過 {model[i]} - {scenarios[i]}，沒有有效的 GWL 年份")
            continue  # 跳過這個模型，避免 NaN 轉換錯誤

        start_year = int(center_yr[i]) - 9 if no != 0 else 1995
        end_year = int(center_yr[i]) + 10 if no != 0 else 2014
        ...
        for year in range(start_year, end_year + 1):
            model_scenarios_year = f"{modelname}_{scenarios[i]}_{year}"
            ...
            if rows:
                ...
                output_filename = f"AR6_統計降尺度_日資料_{variable_c}_{scenarios[i]}_{modelname}_{year}.csv"
